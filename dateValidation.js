...
function isValidDate(date) {
  const parts = isValidFormat(date);
    if (!parts) return false;

    if (!isValidMonth(parts.month) || isValidYear(parts.year)) return false;
    return parts.day > 0 && parts.day <= parts.monthForDay[parts.month - 1];
}
